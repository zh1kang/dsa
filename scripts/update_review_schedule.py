...
def regenerate(config: Config, now: datetime | None = None) -> tuple[dict[str, Any], list[list[str]]]:
    """Rebuild review state, schedule, and CSV. Returns (schedule, sheet rows)."""
    ...
    reviews = fsrs_scheduler.compute_reviews(tracker_data, events, config.desired_retention, now)
    ...
    schedule = fsrs_scheduler.build_schedule(
        root, tracker_data, events, reviews, config.desired_retention, now, config.timezone
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    schedule, _ = regenerate(config, now)
